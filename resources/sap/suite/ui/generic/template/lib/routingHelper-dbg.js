...
sap.ui.define(["sap/f/FlexibleColumnLayout", "sap/ui/model/Filter", "sap/ui/model/FilterOperator", "sap/ui/generic/app/util/ModelUtil",
	...
	function(FlexibleColumnLayout, Filter, FilterOperator, ModelUtil, CRUDHelper, FlexibleColumnLayoutHandler, testableHelper, Targets) {
		 ...
		 */
		function generateRoutingMetadataAndGetRootEntitySet(oNavigationControllerProxy, oNavigationHost) {
			var sNavigationTargetId = oNavigationHost.getId();
			...
			oTopRoute = createRoute([], oTopRouteConfig, "root", 0, null, oNavigationControllerProxy, sNavigationTargetId);
			oNavigationControllerProxy.oRouter.addRoute(oTopRoute);

			createQueryRoute(oTopRoute, oNavigationControllerProxy);

			createChildRoutes(oTopRoute.target, oTopRouteConfig, 0, null, oNavigationControllerProxy, sNavigationTargetId);
		 ...
		 */
		function createChildRoutes(aPredecessorTargets, oRoute, iLevel, oParent, oNavigationControllerProxy, sNavigationTargetId, aNodes) {
			var i, iLen;
			if (oRoute.pages) {
				iLen = oRoute.pages.length;
				for (i = 0; i < iLen; i++) {
					createRoutes(aPredecessorTargets, oRoute.pages[i], (iLevel + 1), oParent, oNavigationControllerProxy, sNavigationTargetId, aNodes);
				}
			}
		}
		 ...
		 * @param {Array} aNodes - optional: Add the names of entity sets for the created roots to this array
		 */
		function createRoutes(aPredecessorTargets, oRoute, iLevel, oParent, oNavigationControllerProxy, sNavigationTargetId, aNodes) {
			if (oRoute.component) { //in case of intent there is no internal route to be created
				...
				var oNewRoute = createRoute(aPredecessorTargets, oRoute, oRoute.component.list ? "aggregation" : "detail", iLevel, oParent,
					oNavigationControllerProxy, sNavigationTargetId);
				...
				createChildRoutes(oNewRoute.target, oRoute, iLevel, oNewRoute, oNavigationControllerProxy, sNavigationTargetId, oTreeNode.children);
			}
		}
		...
		function createTarget(oRouter, sControlId, sTargetName, sViewName, sControlAggregation) {
			var oTarget = {};
			oTarget = {
				viewName: sViewName,
		 ...
		 */
		function createRoute(aPredecessorTargets, oRoute, sOperation, iLevel, oParentRoute, oNavigationControllerProxy, sNavigationTargetId) {
			...
			var sControlId, sTargetName, sControlAggregation;
			if (oNewRoute.viewLevel < 3 && oNavigationControllerProxy.oTemplateContract.oFlexibleColumnLayoutHandler) { // In this case the view is hosted by the FCL
				sControlId = sNavigationTargetId;
				sTargetName = oNewRoute.name;
				switch (oNewRoute.viewLevel) {
					case 0:
						sControlAggregation = "beginColumn";
						break;
					case 1:
						sControlAggregation = "midColumn";
						break;
					case 2:
						sControlAggregation = "endColumn";
				}
				oNewRoute.target = aPredecessorTargets.concat([sTargetName]);
			} else { // In this case the view is hosted by the NavContainer
				sControlId = oNavigationControllerProxy.oTemplateContract.oNavContainer.getId(); // do not use sNavigationTargetId, this would be wrong in the FCL case
				sControlAggregation = "pages";
				sTargetName = oNewRoute.name;
				oNewRoute.target = sTargetName;
			}
			createTarget(oNavigationControllerProxy.oRouter, sControlId, sTargetName, oNewRoute.name, sControlAggregation);
		...
		function fnProcessStartupParameters(oNavigationControllerProxy, sEntitySet, oStartupParameters) {
			...
			oModel.getMetaModel().loaded().then(function() {
				...
				switch (oMode.mode) {
					case "create":
						//App opens with a create view on the data.
						//keys are not relevant/ignored for create
						oNavigationControllerProxy.oTemplateContract.oApplicationProxy.addDataForNextPage({
							addEntry: true,
							addEntryForDeleteButton: true,
							addEntryForCancelButton: true
						});
		...
		function fnStartupRouter(oNavigationControllerProxy) {
			...
			var oNavigationHost;
			if (oManifestEntryGenricApp.settings && oManifestEntryGenricApp.settings.flexibleColumnLayout && oManifestEntryGenricApp.settings.flexibleColumnLayout ===
				true) {
				oNavigationHost = new FlexibleColumnLayout({
					threeColumnLayoutType: "MidColumnEmphasized",
					threeColumnLayoutTypeFixed: oManifestEntryGenricApp.settings.flexibleColumnLayoutIsThirdColumnFixed
				});
				oNavigationControllerProxy.oTemplateContract.oNavContainer.addPage(oNavigationHost);
				oNavigationControllerProxy.oTemplateContract.oFlexibleColumnLayoutHandler = new FlexibleColumnLayoutHandler(
					oNavigationHost, oNavigationControllerProxy
				);
			} else {
				oNavigationHost = oNavigationControllerProxy.oTemplateContract.oNavContainer;
			}

			var sEntitySet = generateRoutingMetadataAndGetRootEntitySet(oNavigationControllerProxy, oNavigationHost);
