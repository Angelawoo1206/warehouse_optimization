...
(function() {
	...
	sap.ui.define(["jquery.sap.global", "sap/ui/core/mvc/ViewType", "sap/ui/core/UIComponent", "sap/ui/model/json/JSONModel", "sap/ui/model/resource/ResourceModel", "sap/ui/Device"],
		 ...
		 * @private
		 */
		function createXMLView(oComponent, fnCreateViewController, aCacheKeys) {
			var oView = null;
			...
			oComponent.runAsOwner(function() {
				var preprocessorsData = oComponent.getComponentData().preprocessorsData;
				var oViewSettings = {
					async: true,
					preprocessors: {
						xml: {
							...
							models: {
								...
								parameter: createParameterModel(oEntitySet.entityType, oComponent, oMetaModel)
							},
					...
					cache: { 
						keys: aCacheKeys,
						additionalData: [preprocessorsData]
					}					
				};
					...
					if (oModel) {
						oModel.getMetaModel().loaded().then(function() { // Do the templating once the metamodel is loaded
							if (!oComponentRegistryEntry.createViewStarted) {
								...
								createXMLView(oComponentRegistryEntry.oComponent, oComponentRegistryEntry.createViewController, aCacheKeys).loaded().then(function(oView) {
