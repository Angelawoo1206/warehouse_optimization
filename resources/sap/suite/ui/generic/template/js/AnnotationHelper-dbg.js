	...
	sap.suite.ui.generic.template.js.AnnotationHelper = {
		...
		containsFormWithBreakoutAction: function (oFacetCandidate, sIdCriterion) {
			...
			if (sCandidateId === sIdCriterion) {
				if (oFacetCandidate.RecordType === "com.sap.vocabularies.UI.v1.ReferenceFacet" &&
						...
						oFacetCandidate.Target.AnnotationPath.includes("com.sap.vocabularies.UI.v1.FieldGroup")) {
					return true;
				}
			} else {
				if (oFacetCandidate.Facets) {
					for (var i in oFacetCandidate.Facets) {
						var oSubFacetCandidate = oFacetCandidate.Facets[i];
						var bResult = sap.suite.ui.generic.template.js.AnnotationHelper.containsFormWithBreakoutAction(oSubFacetCandidate, sIdCriterion);
						if (bResult) {
							return true;
						}
					}
		...
		getSelectionModeGridTable: function(aEntities, sRootEntitySet, oManifestExt, oFacet, oEntitySet, oRouteConfig, bIsDraftEnabled) {
			...
			if (oDeleteRestrictions && oDeleteRestrictions.Deletable) {
				...
				if (oDeleteRestrictions.Deletable.Path) {
					return "{= ${ui>/editable} && ${" + oDeleteRestrictions.Deletable.Path + "} ? 'Single' : 'None' }";
				}
			}
		...
		getSelectionModeResponsiveTable: function(aEntities, sRootEntitySet, oManifestExt, oFacet, oEntitySet, oRouteConfig, bIsDraftEnabled) {
			...
			if (oDeleteRestrictions && oDeleteRestrictions.Deletable) {
				...
				if (oDeleteRestrictions.Deletable.Path) {
					return "{= ${ui>/editable} && ${" + oDeleteRestrictions.Deletable.Path + "} ? 'SingleSelectLeft' : 'None' }";
				}
			}
			//Default mode, only IsDraftEnabled and NO settings in Annotation like sap:deletable="true" or sap:deletable="false"
			if (bIsDraftEnabled) {
		...
		isRelatedEntityCreatable: function (oInterface, oSourceEntitySet, oRelatedEntitySet, aSubPages) {
			...
			if (sap.suite.ui.generic.template.js.AnnotationHelper.hasSubObjectPage(oRelatedEntitySet, aSubPages)) {
				result = "{= ${ui>/editable}}";
