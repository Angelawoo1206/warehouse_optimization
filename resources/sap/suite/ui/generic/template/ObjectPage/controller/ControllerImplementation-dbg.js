				...
				getMethods: function(oViewProxy, oTemplateUtils, oController) {
					...
					function fnAdaptBindingParamsForInlineCreate(oEvent) {
						if (fnIsEventForTableWithInlineCreate(oEvent.getSource())) {
							...
							var fnGroup = oBindingParams.sorter[0] && oBindingParams.sorter[0].fnGroup;
					...
					function fnOnSemanticObjectLinkNavigationTargetObtained(oEvent) {
						...
						var proceedWithClickedField = function(oEvent, oReferentialConstraint){
							...
							//table fields get an absolute id "__link0-__clone34" - then jQuery is used to retrieve this absolut id (jQuery doesn't work with "::")
							if (!oControl && !sClickedFieldId.indexOf("::") > -1){
								var oElement = jQuery( "#" + sClickedFieldId );
								if (oElement){
							...
							//sClickedFieldId endsWith - not covering all cases: __link0-__clone34 in case of smart table fields
							/*
							if (sClickedFieldId.endsWith("::" + sPropertyRefName + "::Field-sl")){
								bProceed = true;
								return bProceed;
							}*/
							
							//check if the clicked field is annotated with an semantic object - not covering all cases
							/*fieldSemanticObjectMap can contain the following
							 Product 		EPMProduct
							 ProductForEdit EPMProduct
							 Supplier		EPMProduct
							 In this case even when ProductForEdit was clicked (for which no refConstraint exist) checking for sPropertyRefName=Supplier
							with oEvent.oSource.mProperties.fieldSemanticObjectMap[sPropertyRefName] === sSemanticObject return true, why here the QuickView is found
							var sSemanticObject = oEvent.mParameters.semanticObject;
							if (oEvent.oSource.mProperties.fieldSemanticObjectMap[sPropertyRefName] && 
									oEvent.oSource.mProperties.fieldSemanticObjectMap[sPropertyRefName] === sSemanticObject){
								return bProceed;
							}
							*/
							return false;
                    ...
                    function computeAndSetVisibleParamsForNavigationBtns() {
                        ...
                        var bPaginatorAvailable = !!oPaginatorInfo && (!bIsContainedInFCL || oTemplatePrivateGlobalModel.getProperty("/generic/FCL/isFullscreen"));
					...
					function fnBackToPrevObject() {
						var oComponent = oController.getOwnerComponent();
						var oTemplatePrivateModel = oComponent.getModel("_templPriv");
						var oDataFromLastPage = oTemplateUtils.oComponentUtils.getCurrentDisplayObject().dataFromLastPage;
						if (oDataFromLastPage && oDataFromLastPage.addEntry) {
							oDataFromLastPage.addEntry = false;
							oTemplatePrivateModel.setProperty("/backToPrevious", oTemplateUtils.oCommonUtils.getText("ADD"));
						} else if (oDataFromLastPage && oDataFromLastPage.currentPageTitle) {
							oTemplatePrivateModel.setProperty("/backToPrevious", oTemplateUtils.oCommonUtils.getText("ST_GENERIC_BACK_TO", oDataFromLastPage.currentPageTitle));
						} else {
							oTemplatePrivateModel.setProperty("/backToPrevious", oTemplateUtils.oCommonUtils.getText("BACK"));
						}
					}

					function fnDeleteButtonOnFooter() {
						var oComponent = oController.getOwnerComponent();
						var oMetaModel = oComponent.getModel().getMetaModel();
						var oModelEntitySet = oMetaModel.getODataEntitySet(oComponent.getEntitySet());
						var bDeletable;
						var oDeleteRestrictions = oModelEntitySet["Org.OData.Capabilities.V1.DeleteRestrictions"];
						if (oDeleteRestrictions && oDeleteRestrictions.Deletable) {
							bDeletable = oDeleteRestrictions.Deletable.Bool;
						}
						var oTemplatePrivateModel = oComponent.getModel("_templPriv");
						var oDataFromLastPage = oTemplateUtils.oComponentUtils.getCurrentDisplayObject().dataFromLastPage;

						if (bDeletable !== 'false' && oDataFromLastPage && oDataFromLastPage.addEntryForDeleteButton !== true) {
							oTemplatePrivateModel.setProperty("/deleteButtonOnFooter", true);
						} else {
							oTemplatePrivateModel.setProperty("/deleteButtonOnFooter", false);
						}
					}

					function fnCancelButtonOnFooter() {
						var oComponent = oController.getOwnerComponent();
						var oMetaModel = oComponent.getModel().getMetaModel();
						var oModelEntitySet = oMetaModel.getODataEntitySet(oComponent.getEntitySet());
						var bDeletable;
						...
						if (bDeletable !== 'false' && oDataFromLastPage && oDataFromLastPage.addEntryForCancelButton) {
							oTemplatePrivateModel.setProperty("/cancelButtonOnFooter", true);
						} else {
							oTemplatePrivateModel.setProperty("/cancelButtonOnFooter", false);
						}
					}
					...
					oViewProxy.onComponentActivate = function(sBindingPath) {
                        ...
                        //prepare text of back button in the footer. E.g. "Back to Product Text", "Back to Product"
                        fnBackToPrevObject();
                        fnDeleteButtonOnFooter();
                        fnCancelButtonOnFooter();
						...
						var sHeaderTitle = oHeaderInfo.TypeName.String;
						...
						oTemplateUtils.oComponentUtils.setTitle(sHeaderTitle);
					...
					oViewProxy.bindBreadCrumbs = function(aSections) {
						if (!aSections.length) {
							return;
						}
					...
					var fnBackToPrevObject = testableHelper.testable(fnBackToPrevObject, "backToPrevObject");
					var fnDeleteButtonOnFooter = testableHelper.testable(fnDeleteButtonOnFooter, "deleteButtonOnFooter");
					var fnCancelButtonOnFooter = testableHelper.testable(fnCancelButtonOnFooter, "cancelButtonOnFooter");
					...
					var oControllerImplementation = {
						...
						handlers: {
							...
							onCancel: function() {
								var sMode = "Proceed";
								if (oTemplateUtils.oComponentUtils.getCreateMode() || !bIsObjectRoot){
									sMode = "LeavePage";
								}
							...
							onSave: function() {
								...
								var bCreateMode = oTemplateUtils.oComponentUtils.getCreateMode();
							...
							onBeforeRebindDetailTable: function(oEvent) {
								oTemplateUtils.oCommonEventHandlers.onBeforeRebindTable(oEvent, oTemplateUtils.oCommonUtils);
