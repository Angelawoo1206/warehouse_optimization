...
function(VizFrame, VizFrameRenderer, FlattenedDataset, DimensionDefinition, MeasureDefinition, FeedItem,
		...
		var dataSet = new FlattenedDataset({
				...
				events: {
					dataReceived: function(ev) {
						//Need to be sure that top 2 query has returned as well.
	...
	FilterItemChartDonut.prototype._onDataReceived = function(ev) {
		//Both top2 records and total has to be ready
		...
		var contextList = dataSet.getBinding("data").getContexts(0, 2);
		//var scaleFactor = this._getMaxScaleFactor(contextList, measureField);
		...
		if (dataSet.getBinding("data").getLength() > 2) { // Only add Other if more than 2 total results
