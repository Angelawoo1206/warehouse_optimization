sap.ui.define([
	...
	"sap/ui/model/Context"
		 ...
		 */
		KpiUtil.getPathOrPrimitiveValue = function (oModel, oItem) {
			if (oItem) {
				if (oItem.Path) {
    ...
    */
	...
	KpiUtil.getUnitofMeasure = function(oModel, oEntityTypeProperty)
	{
		...
		if (oEntityTypeProperty && oEntityTypeProperty["Org.OData.Measures.V1.Unit"] ) {
			...
			result =  KpiUtil.getPathOrPrimitiveValue(oModel, oUnit);
		}
			...
			result =  KpiUtil.getPathOrPrimitiveValue(oModel, oISOCurrency);
		}
		if (result === undefined ) {
				result = "";
		}
