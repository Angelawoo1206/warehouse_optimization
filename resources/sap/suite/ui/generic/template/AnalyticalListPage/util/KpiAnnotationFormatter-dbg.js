	...
	function(KpiUtil) {
		...
		*/
		...
		sap.suite.ui.generic.template.AnalyticalListPage.util.KpiAnnotationFormatter.resolvePathForKpiTargetValue = function(iContext, oDataPoint) {
			...
			var value =  KpiUtil.getPathOrPrimitiveValue(oSettings.model, oDataPoint.Value);
			var targetValue = KpiUtil.getPathOrPrimitiveValue(oSettings.model, oDataPoint.TargetValue);
			...
			if ( oDataPoint.ValueFormat) {
				nNumberOfFractionalDigits = KpiUtil.getPathOrPrimitiveValue(oSettings.model, oDataPoint.ValueFormat.NumberOfFractionalDigits);
				fScaleFactor = KpiUtil.getPathOrPrimitiveValue(oSettings.model, oDataPoint.ValueFormat.ScaleFactor);
			}
			...
			var formatFunc = function() {
				...
				/*} else {
					return calculateTarget(arguments[0],
						bIsTargetValBinding ? arguments[index++] : targetValue,
						nNumberOfFractionalDigits, fScaleFactor);
				}*/
			};
		...
		sap.suite.ui.generic.template.AnalyticalListPage.util.KpiAnnotationFormatter.resolvePathForKpiReferenceValue = function(iContext, oDataPoint) {
			var oSettings = iContext.getSetting("settings").getData();
			var value =  KpiUtil.getPathOrPrimitiveValue(oSettings.model, oDataPoint.Value);
			var referenceValue = KpiUtil.getPathOrPrimitiveValue(oSettings.model, oDataPoint.TrendCalculation.ReferenceValue);
		...
		*/
		sap.suite.ui.generic.template.AnalyticalListPage.util.KpiAnnotationFormatter.resolvePathForKpiValue = function(iContext, oDataPoint) {
			...
			var formatFunc = function() {
				var index = 1;
				//if (bIsPercent) { // will have to relook at this code to avoid string check
		...
		sap.suite.ui.generic.template.AnalyticalListPage.util.KpiAnnotationFormatter.resolvePathForTrendIcon = function(iContext, oDataPoint) {
			...
			if (oDataPoint.Trend) {
				var trend = KpiUtil.getPathOrPrimitiveValue(oSettings.model, oDataPoint.Trend);
				return trend;
			}

			var value = KpiUtil.getPathOrPrimitiveValue(oSettings.model, oDataPoint.Value);
			var referenceValue = KpiUtil.getPathOrPrimitiveValue(oSettings.model, oDataPoint.TrendCalculation.ReferenceValue);
			var downDifference = KpiUtil.getPathOrPrimitiveValue(oSettings.model, oDataPoint.TrendCalculation.DownDifference);
			var upDifference = KpiUtil.getPathOrPrimitiveValue(oSettings.model, oDataPoint.TrendCalculation.UpDifference);
		...
		sap.suite.ui.generic.template.AnalyticalListPage.util.KpiAnnotationFormatter.resolvePathForScale = function(iContext, oDataPoint) {
		...
		function formThePathForCriticalityStateCalculation(iContext, oDataPoint, oCriticalityConfigValues) {
			...
			var deviationLow = oDataPoint.CriticalityCalculation.DeviationRangeLowValue ? KpiUtil.getPathOrPrimitiveValue(oSettings.model,oDataPoint.CriticalityCalculation.DeviationRangeLowValue) : undefined;
			var deviationHigh = oDataPoint.CriticalityCalculation.DeviationRangeHighValue ? KpiUtil.getPathOrPrimitiveValue(oSettings.model,oDataPoint.CriticalityCalculation.DeviationRangeHighValue) : undefined;
			var toleranceLow = oDataPoint.CriticalityCalculation.ToleranceRangeLowValue ? KpiUtil.getPathOrPrimitiveValue(oSettings.model,oDataPoint.CriticalityCalculation.ToleranceRangeLowValue) : undefined;
			var toleranceHigh = oDataPoint.CriticalityCalculation.ToleranceRangeHighValue ? KpiUtil.getPathOrPrimitiveValue(oSettings.model,oDataPoint.CriticalityCalculation.ToleranceRangeHighValue) : undefined;
		...
		sap.suite.ui.generic.template.AnalyticalListPage.util.KpiAnnotationFormatter.getAggregateNumber = function(iContext, oEntitySet, oDataPoint, oSelectionVariant, oSettings) {
			...
			ret += "{path: '" + oSettings.model + ">/" + oEntitySet.name + "'";
			ret += ", length: 1";


			var oEntitySet = oSettings.metaModel.getODataEntitySet(oSettings.entitySet);
		...
		sap.suite.ui.generic.template.AnalyticalListPage.util.KpiAnnotationFormatter.resolvePathForScale.requiresIContext = true;
