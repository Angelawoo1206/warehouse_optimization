...
sap.suite.ui.commons.TimelineRenderer.render = function(oRm, oControl){ 
	...
	if (oControl.getAxisOrientation() === sap.suite.ui.commons.TimelineAxisOrientation.Horizontal){
		...
		if (oControl.getGroup()  && oControl.getGroupBy() != '')
		{
			this.renderVGrouping(oRm, oControl);
		}
		else
		{
...
sap.suite.ui.commons.TimelineRenderer.renderVGrouping = function(oRm, oControl){ 
	var oContent = oControl._outputItem;
	oRm.write("<div ");
	oRm.writeControlData(oControl);
    	...
    	for (var i = 0; i < oContent.length; i++) {
            ...
            if (oC.getText() === 'GroupHeader') {
              ...
              oRm.write("<ul ");
            	...
            	oRm.writeClasses();
            	oRm.writeAttribute("id", oControl.getId() + "-scroll-ul" + i);
            	oRm.writeAttribute("role", "listbox");
            	oRm.write(">"); 
            	 oC._orientation = 'V';
                 oRm.renderControl(oC);  
...
sap.suite.ui.commons.TimelineRenderer.renderV = function(oRm, oControl){ 
	...
	if (!oControl.getEnableScroll()) {
		oRm.addClass("sapSuiteUiCommonsTimelineNoScroll");
	}
	oRm.writeClasses();
	oRm.write(">");
	...
	*/
    ...
    if (oContent.length > 0) {
    	for (var i = 0; i < oContent.length; i++) {
            var oC = oContent[i];
         if (oC.getText() != 'GroupHeader' && oC.getVisible())
		  {
            oC._orientation = 'V';
            oRm.renderControl(oC);
		  }
