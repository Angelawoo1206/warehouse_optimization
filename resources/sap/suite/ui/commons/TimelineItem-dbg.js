	...
	events : {
		...
		"customActionClicked" : {
			parameters : {
				 ...
				 */
				"linkObj" : {type : "sap.ui.core.Control"}
			}
		}, 
...
sap.suite.ui.commons.TimelineItem.prototype.init = function() { 
  ...
  //initialize _showIcons prop.
	...
	this._customActionChange = false;
...
sap.suite.ui.commons.TimelineItem.prototype._getGroupButtonControl = function(type,groupID) {

	var that = this;
	var sImgId = this.getId()+ groupID + '-grptoggle';
	var grpBtn =  new sap.ui.commons.ToggleButton({
		icon : "sap-icon://navigation-down-arrow",
		pressed:true,
		press : function(event) {
		
			
			if (this.getIcon() == "sap-icon://navigation-down-arrow")
			{
				that._performExpandCollapse(groupID,'collapse',event);
				this.setIcon( "sap-icon://navigation-right-arrow");
			}
			else
			{
				that._performExpandCollapse(groupID,'expand',event);
	...
	grpBtn.addStyleClass("sapSuiteUiCommonsTimelineGroupItemIcon"); 
	return grpBtn;
	};
      
sap.suite.ui.commons.TimelineItem.prototype._performExpandCollapse = function(groupID,type,event) {
		
	this.getParent()._performExpandCollapse(groupID,type,event);
	};
sap.suite.ui.commons.TimelineItem.prototype._getImageControl = function(type) {
	var sIconSrc = this.getIcon() ? this.getIcon() : "activity-items";
	var sImgId = this.getId()+ '-icon';
	var mProperties = {src : sIconSrc, tooltip: this.getIconTooltip()};
	var aCssClasses = [ 'sapSuiteUiCommonsTimelineItemIcon' ];
	var cssOdd = [ 'sapSuiteUiCommonsTimelineItemIconOdd' ];
	var cssEven =  [ 'sapSuiteUiCommonsTimelineItemIconEven' ];
	if ( type == 'odd')
	{
	this._oImageControl = sap.m.ImageHelper.getImageControl(sImgId,
	this._oImageControl, this, mProperties, cssOdd,[ 'sapSuiteUiCommonsTimelineItemIcon','sapSuiteUiCommonsTimelineItemIconEven' ]);
	}
	else if ( type == 'even')
	{
	this._oImageControl = sap.m.ImageHelper.getImageControl(sImgId,
			this._oImageControl, this, mProperties, cssEven,[ 'sapSuiteUiCommonsTimelineItemIcon','sapSuiteUiCommonsTimelineItemIconOdd' ]);
	}
	else
	{
		this._oImageControl = sap.m.ImageHelper.getImageControl(sImgId,
			this._oImageControl, this, mProperties, aCssClasses,[ 'sapSuiteUiCommonsTimelineItemIconEven','sapSuiteUiCommonsTimelineItemIconOdd' ]);
	}
...
sap.suite.ui.commons.TimelineItem.prototype._ensureUserNameLinkInitialized = function () {
	if (!this._userNameLink) {
		...
		this._userNameLink = new sap.m.Link({
			...
			press: function (oEvent) {
				that.fireUserNameClicked();
			}
		});
		this._userNameLink.addStyleClass("sapUiSelectable");
	}
};
...
sap.suite.ui.commons.TimelineItem.prototype.setUserNameClickable = function(userNameClickable) {
	...
	this._ensureUserNameLinkInitialized();
};
...
sap.suite.ui.commons.TimelineItem.prototype._isSocialEnabled = function() {
	return this.getParent()
		&& this.getParent().getEnabledSocial
		&& this.getParent().getEnabledSocial();
};
...
sap.suite.ui.commons.TimelineItem.prototype.onBeforeRendering = function() {
	...
	this._jamBar.removeAllContent();
	if (this._isSocialEnabled()) {
		this._jamBar.addContent(this._replyLink);
	}

	this.getCustomAction().forEach(function (customAction) {
		var key = customAction.getKey();
		var value = customAction.getValue();
		var actionLink = new sap.m.Link({
			text: value,
			tooltip: value
		});
		actionLink.addStyleClass("sapSuiteUiCommonsTimelineItemActionLink");
		actionLink.attachPress({
			value: value,
			key: key
		}, function() {
		...
		that._jamBar.addContent(actionLink);
	});
};
...
sap.suite.ui.commons.TimelineItem.prototype._isSameActionList = function(oldActionList, actionList){
	if (oldActionList){
		if (actionList){
			if (oldActionList.length !== actionList.length){
				return false;
			}else {
				for (var i = 0; i < actionList.length; i++) {
					if (actionList[i].getKey() !== oldActionList[i].getKey() || actionList[i].getValue()!== oldActionList[i].getValue()){
						return false;
					}
