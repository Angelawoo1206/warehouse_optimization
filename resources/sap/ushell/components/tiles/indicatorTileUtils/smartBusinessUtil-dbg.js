...
//				});
		...
		getFrontendCache: function(oConfig, oTileApi) {
			var that = this;
			var deferred = sap.ushell.components.tiles.indicatorTileUtils.util.FRONTEND_CACHE_DEFERRED || jQuery.Deferred();
			...
			if (cachedKpiValue) {
				sap.ushell.components.tiles.indicatorTileUtils.util.FRONTEND_CACHE_DEFERRED.resolve(true);
			} else if (sap.ushell.components.tiles.indicatorTileUtils.util.FRONTEND_CACHE_DEFERRED) {
				return sap.ushell.components.tiles.indicatorTileUtils.util.FRONTEND_CACHE_DEFERRED.promise();
			} else {
				sap.ushell.components.tiles.indicatorTileUtils.util.FRONTEND_CACHE_DEFERRED = deferred;
					...
					if (oPageChipCollection) {
						...
						if (frontendCacheQuery) {
							...
							var frontendCacheQueryReafRef = frontendModel.read(cacheQueriUri, null, null, true, function(data) {
								...
								sap.ushell.components.tiles.indicatorTileUtils.util.FRONTEND_CACHE_DEFERRED.resolve(isCached);
							},function(eObject) {
								...
								sap.ushell.components.tiles.indicatorTileUtils.util.FRONTEND_CACHE_DEFERRED.reject();
							});
						} else {
							sap.ushell.components.tiles.indicatorTileUtils.util.FRONTEND_CACHE_DEFERRED.reject();
						}
					} else {
						sap.ushell.components.tiles.indicatorTileUtils.util.FRONTEND_CACHE_DEFERRED.resolve(false);
					}
				});
			}
			return sap.ushell.components.tiles.indicatorTileUtils.util.FRONTEND_CACHE_DEFERRED.promise();
