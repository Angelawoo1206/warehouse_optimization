...
sap.ui.define(['sap/ui/base/ManagedObject', 'sap/ui/rta/controlAnalyzer/ControlAnalyzerFactory', 'sap/ui/dt/ElementUtil',
	...
	var fnGetMoveAction = function(mSettings, oElementDesignTimeMetadata) {
	...
	var fnConfigureMoveCommand = function(oElement, oCommand, mSettings, oElementDesignTimeMetadata){
		var oAction = fnGetMoveAction(mSettings, oElementDesignTimeMetadata);
	...
	CommandFactory.getCommandFor = function(oElement, sCommand, mSettings, oDesignTimeMetadata) {
		...
		var oAction;
		if (mCommand.getAction) {
			oAction = mCommand.getAction(mSettings, oDesignTimeMetadata);
		}

		if (oAction && oAction.changeOnRelevantContainer) {
			mSettings = jQuery.extend(mSettings, {
				element : oDesignTimeMetadata.getRelevantContainer(oConfElement)
			});
			oElement = mSettings.element;
		}
