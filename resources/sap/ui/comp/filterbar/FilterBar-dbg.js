		...
		if (aPersData && aPersData.length) {
			for (i = 0; i < aPersData.length; i++) {
				...
				if (aPersData[i].name === sName) {
					...
					// this.isLiveMode is used to determine if SmartFilterBar is used
...
// }
		...
		for (n in this._mAdvancedAreaFilter) {
			if (n && this._mAdvancedAreaFilter[n].items) {
				...
				for (i = 0; i < this._mAdvancedAreaFilter[n].items.length; i++) {
					...
					oContainer = this._addControlToBasicAreaContainer(oFilterGroupItem, oControl, oLabel, bUseClone);
					if (oContainer) {
						oContainer.setVisible(oFilterGroupItem.getVisible() && oFilterGroupItem.getVisibleInFilterBar());
	 ...
	 * @param {boolean} bIgnoreAriaHandling controls whether aria specific information should be considered
	 * @returns {sap.ui.layout.VerticalLayout} the container
	 */
	FilterBar.prototype._addControlToBasicAreaContainer = function(oFilterItem, oControl, oLabel, bIgnoreAriaHandling) {
		...
		if (oLabel) {
			...
			if (oConditionType && !bIgnoreAriaHandling) {
				oConditionType.setLabel(oLabel);
			} else if (oLabel.setLabelFor && !bIgnoreAriaHandling) {
