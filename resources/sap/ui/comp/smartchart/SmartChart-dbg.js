...
sap.ui.define([
	...
	SmartChart.prototype._updateDrillBreadcrumbs = function() {
		...
		aVisibleDimensionsRev.forEach(function(dim, index, array) {
			...
			if (dim.dimension.length > 0 && typeof this._oChart.getDimensionByName(dim.dimension[dim.dimension.length - 1]) != 'undefined') {
				...
				// Show all dimensions of each drill-stack entry linked together by a "/"
				var sDimLabel = "";
				dim.dimension.forEach(function(dim, index, array) {
					sDimLabel += this._oChart.getDimensionByName(dim).getLabel();
	...
	SmartChart.prototype._updateDetailsText = function(oEvent) {
		...
		if (iSelectedCategories > 0) {
			...
			var sBracketsText = this._oRb.getText("CHART_DETAILSBTN_LABEL_BRACKETS", [iSelectedCategories]);
