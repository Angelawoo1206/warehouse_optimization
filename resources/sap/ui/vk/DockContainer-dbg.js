sap.ui.define([
	...
	DockContainer.prototype.resize = function(w, h, spp) {
		...
		if (this._left != null && this._right != null) {
			// TODO: fix naive resize algorithm
			if (this._dir == 0) {
				var oleft = this._left._elem.offsetHeight;
				var oright = this._right._elem.offsetHeight;
				var mleft = this._left.getMinimumHeight();
				var mright = this._right.getMinimumHeight();	
				var nleft = (spp != null) ? spp : h * oleft / (oleft + oright);
				if (nleft < mleft) {
					nleft = mleft;
				}
				var nright = h - nleft;
				...
				var oleft = this._left._elem.offsetWidth;
				var oright = this._right._elem.offsetWidth;
				var mleft = this._left.getMinimumWidth();
				var mright = this._right.getMinimumWidth();	
				var nleft = (spp != null) ? spp : w * oleft / (oleft + oright);
				if (nleft < mleft) {
					nleft = mleft;
				}
				var nright = w - nleft;
