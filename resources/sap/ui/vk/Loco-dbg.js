...
sap.ui.define([
	...
	Loco.prototype.addHandler = function(handler) {
		...
		var hm = this._handlerMap[viewport];
		
		if (hm == null) {
			this._handlerMap[viewport] = [];
			hm = this._handlerMap[viewport];
			hm.push(handler);
		} else if (hm.indexOf(handler) == -1) {
			hm.push(handler);
		}
	...
	Loco.prototype.removeHandler = function(handler) {
		...
		if (hm != null) {
			var hindex = hm.indexOf(handler);
			if (hindex >= 0) {
				hm.splice(hindex, 1);
			}
		}

		var hindex = h.indexOf(handler);
		if (hindex >= 0) {
			h.splice(hindex, 1);
	 ...
	 * @private
	 */
	Loco.prototype._processClick = function(dblclk, viewport) {
		this._clickTimer = 0;
		...
		if (hm != null) {
			for (var i = hm.length - 1; i >= 0; i--) {
				if (dblclk) {
					var nativeViewportId = hm[i].getViewport().getId();
					...
					if (!parentViewer || !parentViewer.getOverlay() || !(typeof parentViewer.getOverlay().mIACreateCB === "function")) {
						hm[i].doubleClick(event, viewport);
					}
				} else {
					hm[i].click(event, viewport);
				}
			}
		}
	};
	...
	Loco.prototype.endGesture = function(event, viewport) {
		...
		if (!this._touchMoved && !this._isDoubleClick && (this._touchEnd - this._touchStart) < 2000) {
			this._clickTimer = setTimeout(function(that) {
				that._processClick(false, viewport);
			}, 200, this);
		}
