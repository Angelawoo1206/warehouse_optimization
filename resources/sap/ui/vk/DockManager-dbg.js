sap.ui.define([
	...
	DockManager.prototype.dock = function(dp, parent, dir) {
		...
		if (parent == null) {
			...
			var parent = new DockContainer(this, null);
	...
	DockManager.prototype.move = function(event) {
		...
		if (e.n == 1 && this._gesture) {
			...
			// Resize border areas
			if (this._oResize) {
				var dp = this._oResize;
				var od = this._oResizeDir;
				var x = dp._elem.offsetLeft;
				var y = dp._elem.offsetTop;
			...
			if (dp) {
				if (dp._parent == null && dp._tabParent == null && !(this._oIsTab && dp._tabs.length > 0)) {
					// Floating pane move
					var x = dp._elem.offsetLeft + dx;
					var y = dp._elem.offsetTop + dy;
					dp._elem.style.left = x + "px";
					dp._elem.style.top = y + "px";
				} else {
					// Undock from container / tab
					if (Math.abs(this._snapX - e.x) > 16 || Math.abs(this._snapY - e.y) > 16) {
			...
			if (this._dockIcons != null) {
				...
				for (var i = 0; i < this._dockIcons.length; i++) {
