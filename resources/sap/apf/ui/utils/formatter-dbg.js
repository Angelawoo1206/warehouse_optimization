...
(function() {
	...
	sap.apf.ui.utils.formatter = function(oFormatterArgs, metadata, dataResponse) {
		...
		this.getFormattedValue = function(fieldName, originalFieldValue) {
			...
			if (oMetadata.isCalendarYearMonth || oMetadata["sap:semantics"] === "yearmonth" || oMetadata["sap:semantics"] && oMetadata["sap:semantics"].String === "yearmonth" && oMetadata.dataType && oMetadata.dataType.type === "Edm.String") { //calenderYearMonth
				...
				if (originalFieldValue !== null) {
					if (originalFieldValue instanceof Date && oMetadata["sap:display-format"] === "Date") {
						var dateFormatter = sap.ui.core.format.DateFormat.getDateInstance();
						dateFormat = dateFormatter.format(originalFieldValue, false);
			...
			} else if (oMetadata.isCalendarDate || oMetadata["sap:semantics"] === "yearmonthday" || oMetadata["sap:semantics"] && oMetadata["sap:semantics"].String === "yearmonthday" && oMetadata.dataType && oMetadata.dataType.type === "Edm.String") {
				if (originalFieldValue !== null) {
		...
		this.getFormatStringTooltip = function(measure) {
			var self = this;
			...
			var precision = 0;
			precision = this.getPrecision(fieldName);
			formatterInstance.registerCustomFormatter("numberTooltip", function(value) {
				var fixedFloat = sap.ui.core.format.NumberFormat.getFloatInstance({
					style : 'standard',
					minFractionDigits : precision
				});
				var originalFieldValue = fixedFloat.format(value);
				var formattedFieldValue = originalFieldValue;
				var metadataObject = jQuery.extend({}, self.getPropertyMetadata(fieldName));
				formattedFieldValue = self._applyCustomFormatting(metadataObject, fieldName, originalFieldValue, formattedFieldValue);
				return formattedFieldValue;
			});
			sFormatString = "numberTooltip";
		...
		this.getFormatString = function(measure) {
			...
			var precision = 0;
			precision = this.getPrecision(fieldName);
			formatterInstance.registerCustomFormatter("number", function(value) {
				var fixedFloat = sap.ui.core.format.NumberFormat.getFloatInstance({
					style : 'standard',
					minFractionDigits : precision
				});
