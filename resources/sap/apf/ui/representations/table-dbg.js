...
(function() {
	'use strict';
	var oTable, selectedRowScrollTop = 0, isSelectionChanged = false;
	...
	function _drawSelection(oEvent) {
		...
		_updateFilters(this, aFilterValues);
		//		var oAllItemsInTable = oEvent.getSource().getModel().getData().tableData;
	...
	*/
	...
	sap.apf.ui.representations.table.prototype.getSelections = function() {
		...
		var aSelectedItemInTable = _findUniqueFilters(aValidatedFilters, _getAvailableFilterInTableData(this));
	...
	*/
	sap.apf.ui.representations.table.prototype.markSelectionInTable = function(bIsCalledFromTable) {
		...
		var aSelectedFilter = this.UI5ChartHelper.filterValues.map(function(filter) {
			return filter[0];
		});
	...
	*/
	sap.apf.ui.representations.table.prototype.getPrintContent = function(oStepTitle) {
		...
		var oTableForPrint = new sap.ui.layout.VerticalLayout();
		oTableForPrint.addContent(oPrintTable);
		return {
			oTableForPrint : oTableForPrint,
