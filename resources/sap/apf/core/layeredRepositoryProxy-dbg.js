		...
		*/
		...
		function updateTexts(applicationId, callback, doNotReadTexts) {
			...
			if (doNotReadTexts) {
				...
				var promise = textLoading(applicationId, undefined, false);
				promise.then(function(result) {
		...
		function readConfigurationsSync(callback, configuration, application, selectList, options) {
			...
			var mOptions = {
				...
				complete : function(result, status) {
					...
					if (status === "error" || status === "timeout" || status === "abort" || status === "parsererror") {
						callback(undefined, getMetadata(), createErrorMessageObject({ code : '5223', aParameters : [ application ]}, [ status ]));
					} else {
			...
			sRequestPath += connector._buildParams(aParams);
			connector.send(sRequestPath, 'GET', undefined, mOptions);
		}
	};
}());
