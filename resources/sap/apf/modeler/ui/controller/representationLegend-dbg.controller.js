...
(function() {
	...
	sap.apf.modeler.ui.controller.representationDimension.extend("sap.apf.modeler.ui.controller.representationLegend", {
		...
		updatePropertyType : function(sNewProperty) {
			var oController = this;
			var aPropertiesInformation = [], oNewPropertyInfo = {}, nIndex, sDimension;
			...
			for(nIndex = 0; nIndex < aCurrentPropertiesState.length; nIndex++) {
				sDimension = aCurrentPropertiesState[nIndex];
				if (sDimension === oController.oTextReader("none")) {
					continue;
				}
				if (nCurrentViewIndex === nIndex) {
					aPropertiesInformation.push(oNewPropertyInfo);
				} else {
					aPropertiesInformation.push(oController.createCurrentProperiesInfo(sDimension, oNewPropertyInfo));
				}
			}
