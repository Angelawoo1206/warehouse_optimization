...
// we need to look for the beginning of navigation
	 ...
	 *
	 ****************************************************************************************************************************/	
	autoCompleteUtilsLib.prototype.insertOMAlternatives = function(alts, name, prefix, comma) {
		...
		var completion, info;
		for (i = 0; i < alts.length; i++) {
			completion = (name === null) ? alts[i].name : alts[i].name.substring(name.length);
			info = (alts[i].info) ? JSON.stringify(alts[i].info) : null;
			navSuggestions = navSuggestions + comma + '{"text":"' + alts[i].name + 
	...
	autoCompleteUtilsLib.prototype.completeByValueListInLexerError = function(vocaRTServices, parser) {
		...
		if (parser.lexerErrorOccurred) {
			...
			if (altValueList.length > 0) {
				...
				var delimiter = "";
				valListSuggestions += this.insertOMAlternatives(altValueList, this.parsingOutputInfo.lastText, "", delimiter);
				...
				if (this.parsingOutputInfo.contextInfo.contextNavigation.navigation.indexOf(startInput) === 0) {
					...
					if (tokenArray.length > 2) {
						...
						if (contextNavDetails.isValid === false && contextNavDetails.attribute.altAttr.length > 0) {
							this.navigationCompletionInfo.navSuggestions += this.insertOMAlternatives(contextNavDetails.attribute.altAttr,
	...
	autoCompleteUtilsLib.prototype.completeNavSuggestions_dotMode = function(vocaRTServices, parser, prefix) {
		...
		if (navigationDetails.isValid === false) {
			if (navigationDetails.root.isValid === false) {
				...
				this.navigationCompletionInfo.navSuggestions += this.insertOMAlternatives(this.navigationCompletionInfo.navAlternatives, navigationDetails.root.name, prefix, this.navigationCompletionInfo.commaDelimiter);
				this.navigationCompletionInfo.navAlternatives = [];
			} else { // Example: player.
				...
				this.navigationCompletionInfo.navSuggestions += this.insertOMAlternatives(navigationDetails.attribute.altAttr.concat(this.navigationCompletionInfo.navAlternatives), navigationDetails.attribute.name, prefix, this.navigationCompletionInfo.commaDelimiter);
				this.navigationCompletionInfo.navAlternatives = [];
			}
		}
	};	
	...
	autoCompleteUtilsLib.prototype.completeNavSuggestions_termsMode = function(vocaRTServices, parser, prefix, lastToken) {
		...
		if (this.navigationCompletionInfo.navAlternatives.length > 0) {
			this.navigationCompletionInfo.navSuggestions += this.insertOMAlternatives(this.navigationCompletionInfo.navAlternatives, this.navigationCompletionInfo.navigationTextInput, prefix, this.navigationCompletionInfo.commaDelimiter);
			this.navigationCompletionInfo.navAlternatives = [];
		}
