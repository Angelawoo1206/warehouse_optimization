...
//		case parserTokens.NUMBER_OF_OCCURENCES_REGEXP:				
	...
	//
	parseUtilsLib.prototype.TokenInfo = function(tokenName, tokentype, info, startIndex, endIndex, valueListName) {
	...
	function combineSplittedTokens(parseUtilsInstance, expression, modelManager, tokens, index, nextTokenType) {
		...
		if (nextToken && nextToken.type === nextTokenType) {
			...
			tokenType = new parseUtilsInstance.TokenInfo(name, typeObject.group, typeObject.info, currentToken.start, nextToken.stop, null);
		}
		return tokenType;
	}
	...
	parseUtilsLib.prototype.buildTokenTypes = function(parser, originalExpression, modelManager) {
		...
		if (tokens.length === 0) {
			...
			tokenType = new this.TokenInfo(expression, constantsLib.tokenTypesEnum.unknown, null, 0, n, null);
		...
		while (i < tokens.length) {
			...
			if (parser.lexerErrorOccurred && parser.lexerErrorStartIndex === start) {
				//tokenType = new this.TokenInfo(name, (parser.lastToken ?  tokenTypesEnum.reservedWord :  tokenTypesEnum.unknown), null, start, expressionLen);
				name = expression.substring(start, expressionLen + 1);
				tokenType = new this.TokenInfo(name, constantsLib.tokenTypesEnum.unknown, null, start, expressionLen, null);
				tokenTypes.push(tokenType);
				break;
				...
				if (splittedTokens.hasOwnProperty(token.type)) {
					...
					if (tokenType === null) {
						tokenType = new this.TokenInfo(name, typeObject.group, typeObject.info, start, end, valueListName);
						i++;
					}
					else {
						i = i+3;
	...
	parseUtilsLib.prototype.getNavigationObjectFromPath = function(navigationPath, key, modelManager) {
		...
		var navigationObject = vocabularyUtilLib.validateNavigationDetails(navigationPath, modelManager.vocaRTServ, vocaName, [], valueListAttr);
