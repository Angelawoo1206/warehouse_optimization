...
sap.rules.ui.parser.businessLanguage.lib.vocabularyUtils.lib = (function() {
	...
	****************************************************************/
	 ...
	 vocabularyUtilsLib.prototype.getAltTypedTerms = function(vocaRTServ, vocaName, startWith, typeArr, context, isCurrent, completeCollection, isDeprecatedAllContext, isAutoComplete, isInsideWhere, isAllContext) {
			
			var termsObj = vocaRTServ.getTerms(vocaName, startWith);
		    ...
		    for (i=0; i<termArr.length; i++) {
				...
				if (typeArr.length === 0){
				 	termsAlt.push (termInfo.description);
				 	continue;
				}
	...
	****************************************************************/
	...
	/****************************************************************
	 ****************************************************************/
	 vocabularyUtilsLib.prototype.isTermPrefix = function(term_prefix, vocaRTServ, vocaName) {
		 if (!vocaRTServ) {
			 return false;
		 }
		 
		 var terms = this.getAltTypedTerms(vocaRTServ, vocaName,term_prefix, []);	
		if (terms.length > 0) {
			return true;
		}
	 ...
	 * 
	 ****************************************************************/
	 vocabularyUtilsLib.prototype.validateNavigationDetails = function(navDetails, vocaRTServ, vocaName, typeArr, valueListAttribute) {
		 var result = {
				...
				var term = vocaRTServ.getTerm(vocaName, navDetails);
				if (term !== null) {
	 ...
	 ****************************************************************/
	 ...
	 vocabularyUtilsLib.prototype.getNavigationCompletion = 
		 function(startWith, vocaRTServ, vocaName, typeArr, valueListAttribute, context, isCurrent, completeCollection, isDeprecatedAllContext, isInsideWhere, isAllContext) {
		var result = {
				altDO: [],
				altAlias: [],
				altValueList: []
		...
		isCurrent = (!isCurrent ? false: isCurrent);
		result.altDO    = this.getAltTypedTerms(vocaRTServ, vocaName, startWith, typeArr, context, isCurrent, completeCollection, isDeprecatedAllContext, true, isInsideWhere, isAllContext);
