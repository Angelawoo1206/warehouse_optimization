sap.ui.define([], function() {
	...
	Rearrange.prototype._resizeMoveHandler = function(actionObject) {
        ...
        var oCard = this.layoutUtil.dashboardLayoutModel.getCardById(this.layoutUtil.getCardId(actionObject.element.id));
        if (oCard.template === "sap.ovp.cards.stack") {
            return;
        }
		...
		if (actionObject.element) {
            ...
            //To calculate minimum rowspan required to display the header of card
            var cardId = $elem.children().first().attr("id");
            ...
            if (oCompInst) {
                ...
                var iHeaderHeight = this.layoutUtil.getItemHeight(oGenCardCtrl, 'ovpCardHeader');
                var iMinRowSpan = Math.ceil(iHeaderHeight / this.layoutUtil.ROW_HEIGHT_PX);
            }
            ...
            //Card can be resized to minimum of header height of card
            this.resizeData.rowSpan = Math.round(ghostHeightCursor / this.layoutUtil.getRowHeightPx());
            if (this.resizeData.rowSpan < iMinRowSpan) {
                this.resizeData.rowSpan = iMinRowSpan;
            }
