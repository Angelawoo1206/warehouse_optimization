sap.ui.define([], function() {
		...
		/**
		 * set a card to a given grid cell
		 *
		 * @method moveCardToGrid
		 * @param {String} floaterId - ID of the floater card
		 * @param {Object} cell - where to set the card to
		 * @param {Boolean} bPushHorizontal - wether to push affected cards horizontally
		 * @param {Object} oFloaterCard - floater card object
		 * @param {Boolean} bInProgress - function is called internally (reuse), model not stable -> do not update variant
		 * @returns {Array} aAffectedCards - what cards are affected by card move action
		 */
		...
		/**
		 * calculate the size of the push projection area
		 * get the x and y projection size to determin which cards are affected by floater positioning
		 * 
		 * @method _getPushGrid
		 * @param {Array} aCards - cards in Layout
		 * @returns {Object} oPushGrid
		 */
		...
		/**
		 * collaps succeeding empty rows in layout to one row
		 *
		 * @method condenseEmptyRows
		 * @returns {Array} aAffectedCards - array of cards
		 */
