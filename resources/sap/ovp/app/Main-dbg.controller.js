    ...
    sap.ui.controller("sap.ovp.app.Main", {
            ...
            setTimeout(
                function () {
                    ...
                    for (var i = 0; i < this.aOrderedCards.length; i++) {
                        if (this.aOrderedCards[i].visibility) {
                            oCard = this
                                ._getCardFromManifest(this.aOrderedCards[i].id);
                            ...
                            if (oCard) {
                                oCard.settings.baseUrl = this
                                    ._getBaseUrl();
                                this
                                    ._initCardModel(oCard.model);
                                this
                                    ._loadCardComponent(oCard.template);
                                this.createCard(oCard);
                            }
                        }
                    }
        ...
        _overrideCardModelRead: function (oModel) {
            ...
            oModel.read = function () {
                ...
                //Get visible filter names
                var oGlobalFilter = that.getGlobalFilter();
                var aFieldNames = [], aVisibleFilterItems = oGlobalFilter.getAllFilterItems(true);

                if (aVisibleFilterItems) {
                    var oItem, iLen = aVisibleFilterItems.length;
                ...
                //Get filter data for above retrieved fields
                var aFilters = oGlobalFilter.getFilters(aFieldNames);
